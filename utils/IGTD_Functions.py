...
def table_to_image(norm_d, scale, fea_dist_method, image_dist_method, save_image_size, max_step, val_step, normDir,
    ...
    # Convertir la matriz a rango [0,255]
    img_gray = np.max(ranking_feature) - ranking_feature
    img_norm = (255 * (img_gray - np.min(img_gray)) / (np.ptp(img_gray))).astype(np.uint8)

    # Replicar el canal en R, G, B
    img_rgb = np.stack([img_norm] * 3, axis=-1)

    # Guardar como PNG
    plt.imsave(normDir + '/original_feature_ranking.png', img_rgb)
    plt.close(fig)
    ...
    fig = plt.figure(figsize=(save_image_size, save_image_size))# Convertir la matriz a rango [0,255]
    img_gray = np.max(ranking_feature) - ranking_feature
    img_norm = (255 * (img_gray - np.min(img_gray)) / (np.ptp(img_gray))).astype(np.uint8)
    ...
    # Guardar como PNG
    plt.imsave(normDir + '/image_feature_ranking.png', img_rgb)
    plt.close(fig)
    ...
    # Guardar como PNG
    plt.imsave(normDir + '/optimized_feature_ranking.png', img_rgb)
    plt.close(fig)
...
def multi_generate_image_data(data_list, index, num_row, num_column, coord, image_folder=None, file_name=''):
    ...
    for j in range(num_channel):
        ...
        for i in range(num_sample):
            ...
            image_data[j, :, :, i] = 255 - image_data[
                j, :, :, i]  # High values in the array format of image data correspond
            ...
            if image_folder is not None:
                ...
                pd.DataFrame(image_data[j, :, :, i], index=None, columns=None).to_csv(image_folder + '/' + file_name +
                                                                                      '_' + samples[
                                                                                          i] + '_data_channel_' + str(
                    j) + '.txt', header=None, index=None, sep='\t',
                                                                                      line_terminator='\r\n')
...
def multi_table_to_image(norm_d_list, weight_list, fea_dist_method_list, scale, image_dist_method, save_image_size,
    ...
    data, samples = multi_generate_image_data(data_list=norm_d_list, index=index[min_id, :], num_row=scale[0],
                                              num_column=scale[1], coord=coordinate, image_folder=normDir + '/data',
                                              file_name='')
